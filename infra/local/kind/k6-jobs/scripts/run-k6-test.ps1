...
if (-not $TestName -or -not $Profile) {
    ...
    Write-Host "  -BaseUrl      Optional. Base URL of SUT (default: http://perf-sut-service.perf-sut:8080)"
    ...
    Write-Host "  powershell -File run-k6-test.ps1 -TestName search -Profile regression"
    Write-Host "  powershell -File run-k6-test.ps1 -TestName checkout -Profile stress -TimeoutSeconds 1800"
    Write-Host ""
    Write-Host "Via Make:" -ForegroundColor Yellow
    Write-Host "  make run-k6-smoke"
    Write-Host "  make run-k6-regression"
    Write-Host "  make run-k6-test TEST=checkout PROFILE=smoke"
    exit 1
}
...
if (-not (Test-Path $testScriptPath)) {
    ...
    $testDirs = Get-ChildItem (Join-Path $K6Dir "tests") -Directory | ForEach-Object { $_.Name }
...
if (-not (Test-Path $workloadConfigPath)) {
    ...
    $profileDirs = Get-ChildItem (Join-Path $K6Dir "workloads") -Directory | ForEach-Object { $_.Name }
...
$testScriptContent = Get-Content $testScriptPath -Raw
$workloadContent = Get-Content $workloadConfigPath -Raw
...
if ($podName -and $podName -notmatch "Error") {
    ...
    cmd /c "kubectl cp perf-generators/$podName:/results/results.json `"$runOutputDir/results.json`" 2>&1" | Out-Null
    cmd /c "kubectl cp perf-generators/$podName:/results/summary.json `"$runOutputDir/summary.json`" 2>&1" | Out-Null
