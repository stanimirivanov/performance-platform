...
class TestRuns(Base):
    ...
    status: Mapped[TestStatus] = mapped_column(
        ...
        nullable=False,
        server_default=text("'pending'::metadata.test_status"),
    )
    ...
    test_script: Mapped[str | None] = mapped_column(Text)
    test_profile: Mapped[str | None] = mapped_column(Text)
    start_time: Mapped[datetime.datetime | None] = mapped_column(DateTime(True))
    end_time: Mapped[datetime.datetime | None] = mapped_column(DateTime(True))
    ...
    ci_build_id: Mapped[str | None] = mapped_column(Text)
    ci_job_id: Mapped[str | None] = mapped_column(Text)
    success_rate: Mapped[float | None] = mapped_column(Double(53))
    average_response_time_ms: Mapped[float | None] = mapped_column(Double(53))
    percentiles: Mapped[dict | None] = mapped_column(JSONB)
    error_count: Mapped[int | None] = mapped_column(Integer, server_default=text("0"))
    total_requests: Mapped[int | None] = mapped_column(Integer, server_default=text("0"))

    correlation_events: Mapped[list["CorrelationEvents"]] = relationship(
        "CorrelationEvents", back_populates="run"
    )
    data_artifacts: Mapped[list["DataArtifacts"]] = relationship(
        "DataArtifacts", back_populates="run"
    )
    environments: Mapped["Environments"] = relationship(
        "Environments", uselist=False, back_populates="run"
    )
    resource_snapshots: Mapped[list["ResourceSnapshots"]] = relationship(
        "ResourceSnapshots", back_populates="run"
    )
...
class CorrelationEvents(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["parent_event_id"],
            ["metadata.correlation_events.event_id"],
            name="correlation_events_parent_event_id_fkey",
        ),
        ForeignKeyConstraint(
            ["run_id"],
            ["metadata.test_runs.run_id"],
            ondelete="CASCADE",
            name="correlation_events_run_id_fkey",
        ),
    ...
    phase_name: Mapped[TestPhase | None] = mapped_column(
        Enum(
            TestPhase,
            values_callable=lambda cls: [member.value for member in cls],
            name="test_phase",
    ...
    parent_event: Mapped[Optional["CorrelationEvents"]] = relationship(
        "CorrelationEvents", remote_side=[event_id], back_populates="parent_event_reverse"
    )
    parent_event_reverse: Mapped[list["CorrelationEvents"]] = relationship(
        "CorrelationEvents", remote_side=[parent_event_id], back_populates="parent_event"
    )
...
class DataArtifacts(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["metadata.test_runs.run_id"],
            ondelete="CASCADE",
            name="data_artifacts_run_id_fkey",
        ),
    ...
    data_size_bytes: Mapped[int | None] = mapped_column(BigInteger)
...
class Environments(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["metadata.test_runs.run_id"],
            ondelete="CASCADE",
            name="environments_run_id_fkey",
        ),
        PrimaryKeyConstraint("environment_id", name="environments_pkey"),
        UniqueConstraint(
            "run_id", "fingerprint_hash", name="environments_run_id_fingerprint_hash_key"
        ),
    ...
    kubernetes_version: Mapped[str | None] = mapped_column(Text)
    cloud_provider: Mapped[str | None] = mapped_column(Text)
    cloud_region: Mapped[str | None] = mapped_column(Text)
    cloud_zone: Mapped[str | None] = mapped_column(Text)
    node_count: Mapped[int | None] = mapped_column(Integer)
    node_os: Mapped[str | None] = mapped_column(Text)
    node_kernel: Mapped[str | None] = mapped_column(Text)
    node_architecture: Mapped[str | None] = mapped_column(Text)
    node_resource_capacity: Mapped[dict | None] = mapped_column(JSONB)
...
class ResourceSnapshots(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["metadata.test_runs.run_id"],
            ondelete="CASCADE",
            name="resource_snapshots_run_id_fkey",
        ),
    ...
    value_min: Mapped[float | None] = mapped_column(Double(53))
    value_max: Mapped[float | None] = mapped_column(Double(53))
    value_avg: Mapped[float | None] = mapped_column(Double(53))
    value_current: Mapped[float | None] = mapped_column(Double(53))
    unit: Mapped[str | None] = mapped_column(Text)
    test_phase: Mapped[TestPhase | None] = mapped_column(
        Enum(
            TestPhase,
            values_callable=lambda cls: [member.value for member in cls],
            name="test_phase",
